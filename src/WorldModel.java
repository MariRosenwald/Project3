...
final class WorldModel
{
   ...
   private PathingStrategy Lstrat = new NewPath();
...
*/
   ...
   public Point nextPointL(LeadChaser c, Player p, boolean Idied){
      //System.out.println(getOccupancyCell(po));
      try
      {
         List<Point> points = Lstrat.computePath(c.getPosition(), p.getPosition(),
                 po ->  withinBounds(po) && !this.isOccupied(po),
                 (p1, p2) -> neighbors(p1,p2),
                 PathingStrategy.CARDINAL_NEIGHBORS);
         return points.get(0);
